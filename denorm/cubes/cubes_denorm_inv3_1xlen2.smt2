...
; Benchmark: 		cubes_denorm_inv3_1xlen2
...
; 		 1 - 3*c + k*n - k + 2*n
; 		 1 + 3*n^2 - k + 3*n
; Testing invariant: (1 - 3*c + k*n - k + 2*n) * (c^2*n + c^3*k*n^2)
;--------------------------------------------------------------------------
...
(assert 
	(exists
		...
		(and
			...
			(or
				(not (= (* (+ (- (+ (- 1 (* 3 (+ c k))) (* (+ k m) (+ n 1))) (+ k m)) (* 2 (+ n 1))) (+ (* (+ c k) (+ c k) (+ n 1)) (* (+ c k) (+ c k) (+ c k) (+ k m) (+ n 1) (+ n 1)))) 0))
			)
		)
	)
)
