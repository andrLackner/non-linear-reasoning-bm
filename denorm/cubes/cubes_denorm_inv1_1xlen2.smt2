...
; Benchmark: 		cubes_denorm_inv1_1xlen2
...
; 		 1 - 3*c + k*n - k + 2*n
; 		 1 + 3*n^2 - k + 3*n
; Testing invariant: (-6*1 + m - 6*n) * (-c^2*k^2*m^2*n^2 + c^2*k^2)
;--------------------------------------------------------------------------
...
(assert 
	(exists
		...
		(and
			...
			(or
				(not (= (* (- (+ (* (- 6) 1) m 6) (* 6 (+ n 1))) (+ (* (- 1) (+ c k) (+ c k) (+ k m) (+ k m) (+ m 6) (+ m 6) (+ n 1) (+ n 1)) (* (+ c k) (+ c k) (+ k m) (+ k m)))) 0))
			)
		)
	)
)
