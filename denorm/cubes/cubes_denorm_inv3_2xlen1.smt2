...
; Benchmark: 		cubes_denorm_inv3_2xlen1
...
; 		 1 - 3*c + k*n - k + 2*n
; 		 1 + 3*n^2 - k + 3*n
; Testing invariant: (1 - 3*c + k*n - k + 2*n) * (k) * (c^2*k^2*m^2*n^2)
;--------------------------------------------------------------------------
...
(assert 
	(exists
		...
		(and
			...
			(or
				(not (= (* (+ (- (+ (- 1 (* 3 (+ c k))) (* (+ k m) (+ n 1))) (+ k m)) (* 2 (+ n 1))) (+ k m) (+ c k) (+ c k) (+ k m) (+ k m) (+ m 6) (+ m 6) (+ n 1) (+ n 1)) 0))
			)
		)
	)
)
